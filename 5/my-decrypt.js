...
// const decodeCode = require('./decode');
...
if (!config.only_decrypt) {
    ...
    traverse(ast, {ExpressionStatement: decodeString,});           // 字符串解码
...
function step1(ast) {
    ...
    traverse(ast, {
        BlockStatement: (path) => {
            if(i === 0){
                decrypt_code = path.node.body.slice(0, 3)
                rest_code = path.node.body.slice(3)
                i++;
            }
        }
    })
...
function step2(ast) {
    ...
    console.log("还原解密函数中...")
    fs.writeFileSync('./g.js', global_code)
    // 利用eval将前3个节点的代码导入到环境中
    // global_code = fs.readFileSync('./decode.js')
    eval(global_code)
    traverse(ast, {
    ...
    function funToStr(path) {
        var node = path.node;
        // 判断节点类型及函数名，不是则返回
        if (decryptStr.every(x => !t.isIdentifier(node.callee, {name: x }))) 
            return;
        // 调用解密函数
